...
@ddt
class TouristsLogin(unittest.TestCase):
    """
    初始化数据，从Excel中拿到数据
    """
    excel = OperateExcel("TouristsLogin")
    ...
    @data(*cases)
    def test_tourists_login(self, cases):
        ...
        try:
            # 数据均正常情况请求
            if 'datas' in test_res.keys() and test_res['datas']['accessToken']:
