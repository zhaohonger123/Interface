...
@ddt
class BonusRecords(unittest.TestCase):
    excel = OperateExcel("BonusRecords")
    ...
    @data(*cases)
    def test_bonus_records(self, cases):
        log.info(">>>>开始组装请求头数据<<<<")
        ...
        try:
            # 数据均正常情况请求
            if 'datas' in test_res.keys() and test_res['datas']['total'] is not None:
                self.assertEqual(3.75, test_res['datas']['total'])
