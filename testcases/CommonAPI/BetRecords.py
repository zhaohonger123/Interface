...
@ddt
class BetRecords(unittest.TestCase):
    excel = OperateExcel("BetRecords")
    ...
    @data(*cases)
    def test_bet_records(self, cases):
        log.info(">>>>开始组装请求头数据<<<<")
        ...
        try:
            # 数据均正常情况请求
            if 'datas' in test_res.keys() and test_res['datas']['betTotal'] is not None:
                self.assertEqual(-5.00, test_res['datas']['betTotal']['winLossMoney'])
