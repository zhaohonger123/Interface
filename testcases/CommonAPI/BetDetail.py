...
@ddt
class BetDetail(unittest.TestCase):
    """
    初始化数据，从Excel中拿到数据
    """
    excel = OperateExcel("BetDetail")
    ...
    @data(*cases)
    def test_bet_detail(self, cases):
        ...
        try:
            # 数据均正常情况请求
            if 'datas' in test_res.keys() and 'betMoney' in test_res['datas']:
            ...
            # 其他情况
            elif test_res['resp_code'] == 0:
                self.assertEqual('succeed', test_assert)
                self.excel.write_excel(cases["id"] + 1, test_response, "pass")
                log.info("当前断言部分：预期:{},实际：{}".format(cases["expected result"], test_res))
            else:
                log.error("请求失败 {}".format(test_response))
                self.fail(msg='用例执行失败')
