...
@ddt
class GetMoneyChangeList(unittest.TestCase):
    excel = OperateExcel("GetMoneyChangeList")
    ...
    @data(*cases)
    def test_get_money_change_lists(self, cases):
        log.info(">>>>开始组装请求头数据<<<<")
        ...
        try:
            # 数据均正常情况请求
            if 'datas' in test_res.keys() and test_res['datas']['count'] is not None:
                self.assertEqual(3, test_res['datas']['count'])
            ...
            # 其他情况
            elif test_res['resp_code'] == 1 and test_assert == 'failed':
                self.assertEqual('failed', test_assert)
                self.excel.write_excel(cases["id"] + 1, test_response, "pass")
                log.info("当前断言部分：预期:{},实际：{}".format(cases["expected result"], test_res))
            elif test_assert != 'failed':
                self.assertIn('cannot be empty', test_assert)
                self.excel.write_excel(cases["id"] + 1, test_response, "pass")
                log.info("当前断言部分：预期:{},实际：{}".format(cases["expected result"], test_res))
            else:
